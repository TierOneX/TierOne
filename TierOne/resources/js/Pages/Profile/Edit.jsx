...
import { FileText, ExternalLink, Package, Download } from "lucide-react";
                            ...
                            <div className="px-5 pb-5 border-t border-white/[0.05] space-y-4 pt-4">
                                <div className="space-y-3">
                                    {(orden.items ?? []).map((item) => (
                                        <div
                                            key={item.id}
                                            className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/[0.02] p-3 rounded-lg border border-white/[0.03]"
                                        >
                                            <div className="flex items-center gap-3">
                                                {item.producto?.imagen_principal ? (
                                                    <img 
                                                        src={`/storage/${item.producto.imagen_principal}`} 
                                                ...
                                                <div>
                                                    <Link 
                                                        href={item.producto ? route('product.show', item.producto.slug) : '#'}
                                                        className="text-xs font-bold text-white hover:text-[#e31837] transition-colors flex items-center gap-1.5"
                                                    >
                                                    ...
                                                    <div className="flex flex-wrap gap-2 mt-1">
                                                        {item.variante && (
                                                            <span className="text-[9px] uppercase tracking-widest font-black text-gray-500">
                                                                Modelo: {item.variante.nombre}
                                                            </span>
                                                        )}
                                                        {item.personalizacion_data && (
                                                            <span className="text-[9px] uppercase tracking-widest font-black text-red-500/80 bg-red-500/5 px-1.5 rounded">
                                                                Personalizado
                                                            </span>
                                                        )}
                                                    </div>
                                            ...
                                            <div className="flex items-center justify-between sm:justify-end gap-6 text-right">
                                                <div className="text-left sm:text-right">
                                                    <p className="text-[9px] text-gray-600 uppercase tracking-widest font-bold">Cantidad</p>
                                                    <p className="text-xs text-gray-400">×{item.cantidad}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[9px] text-gray-600 uppercase tracking-widest font-bold">Subtotal</p>
                                                    <p className="text-xs text-white font-black">{money(item.subtotal)}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 border-t border-white/[0.05]">
                                    {orden.tracking_number ? (
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                                            <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">
                                                Seguimiento: <span className="text-white font-mono">{orden.tracking_number}</span>
                                                {orden.transportista && ` (${orden.transportista})`}
                                            </p>
                                        </div>
                                    ) : (
                                        <div className="text-[10px] text-gray-600 uppercase tracking-widest font-bold italic">
                                            Envío pendiente de procesar
                                        </div>
                                    )}

                                    <a
                                        href={route('order.invoice', orden.id)}
                                        target="_blank"
                                        className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[10px] font-black uppercase tracking-widest text-white transition-all"
                                    >
