...
export default function GameCard({ juego }) {
    ...
    return (
        ...
        >
            {/* Wrapper que se mueve — NO tiene overflow-hidden */}
            <div className="relative aspect-[3/4] w-full transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-primary/20 shadow-xl">
                {/* Contenedor que recorta — NO se transforma nunca */}
                <div className="absolute inset-0 overflow-hidden rounded-2xl bg-[#1A1A1A]" style={{ transform: 'translateZ(0)' }}>
                    <img 
                    ...
                    {rating > 0 && (
                        <div className="absolute right-3 top-3 flex items-center gap-1 rounded-xl bg-black/60 px-2 py-1 text-xs font-black backdrop-blur-md shadow-lg">
            ...
            <div className="mt-4 px-1">
                ...
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {juego.genres && juego.genres.slice(0, 2).map((genre, idx) => (
                        <span key={idx} className="rounded-md bg-white/5 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-white/40">
                            {genre}
                        </span>
                    ))}
                </div>
